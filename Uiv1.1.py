...
class Example(QWidget):
    ...
    def sample(self):
        ...
        if os.path.exists(model_path+"/"+name)==0:
            ...
            cap.release()
            cv2.destroyAllWindows()
            if os.path.exists(model_path+"/"+name+ "/embedding.txt") == 0:
                embedding_image(model_path+"/"+name, self.images_placeholder, self.embeddings, self.phase_train_placeholder,
                                self.sess)
                train_model(model_path+"/"+name)
